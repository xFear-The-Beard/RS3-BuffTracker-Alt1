/**
 * Custom digit reader for the RS3 dual-text upper-left timer (compact font).
 *
...
import { UPPER_DIGIT_TEMPLATES, UpperDigitTemplate } from './digit-templates-upper';

const BRIGHT_THRESHOLD_STRICT = 180;
const BRIGHT_THRESHOLD_RELAXED = 150;
const MATCH_THRESHOLD = 0.80;

export function readUpperTimerFromPixels(
    buffer: ImageData,
    ox: number,
    oy: number,
    ...
    const strict = readUpperTimerWithThreshold(buffer, ox, oy, scanDirection, gridSize, BRIGHT_THRESHOLD_STRICT);
    const relaxed = readUpperTimerWithThreshold(buffer, ox, oy, scanDirection, gridSize, BRIGHT_THRESHOLD_RELAXED);
...
/**
 * Internal: read upper-left timer at a specific brightness threshold.
 */
function readUpperTimerWithThreshold(
    buffer: ImageData,
    ox: number,
    oy: number,
    ...
    try {
        ...
        // Sliding-window template matching against compact upper-left templates
        raw = matchTemplatesUpper(bmp, w, textTop, textBottom);
        ...
        console.log(`[DigitReaderUpper] Error: ${e}`);
    }
...
/**
 * Greedy left-to-right template matching across the text band, using the
 * compact upper-left templates only.
 */
function matchTemplatesUpper(
    bmp: number[][],
    bmpW: number,
    textTop: number,
    textBottom: number,
    ...
    while (x <= lastX && guard++ < 50) {
        ...
        for (const [label, variants] of Object.entries(UPPER_DIGIT_TEMPLATES)) {
            for (const tpl of variants) {
                if (x + tpl.w > bmpW) continue;
                const score = scoreTemplateUpper(bmp, x, textTop, tpl);
                if (score > bestScore) {
                    bestScore = score;
                    bestLabel = label;
                    bestTplW = tpl.w;
